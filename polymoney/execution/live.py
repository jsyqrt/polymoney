...
class LiveExecutor(OrderExecutor):
    ...
    def query_trade_costs(
        self, market_id: str, side: str, reference_price: float,
    ) -> Optional[Tuple[float, float, float, float]]:
        """Query all confirmed trades for one side of a market.

        Computes accurate buy cost and sell proceeds from the Polymarket
        Trades API, handling complement pricing via *reference_price*.

        Args:
            market_id: Market slug.
            side: ``"up"`` or ``"down"``.
            reference_price: Current mid-price for this side, used to
                resolve complement-pricing ambiguity.

        Returns:
            ``(buy_shares, buy_cost, sell_shares, sell_proceeds)`` or
            ``None`` on failure.
        """
        config = self._markets.get(market_id)
        if not config or not self._client or not self._maker_address:
            return None

        token_id = config.up_token_id if side == "up" else config.down_token_id
        ...
        try:
            ...
            _OK = {
                "CONFIRMED", "MINED", "MATCHED",
                "TRADE_STATUS_CONFIRMED", "TRADE_STATUS_MINED",
            ...
            if trades is None or not isinstance(trades, list):
                return None
            # Empty list is valid — means no trades for this token.
            ...
            for trade in trades:
                if trade.get("status", "") not in _OK:
                    continue
                try:
                    size = float(trade.get("size", 0))
                    raw_price = float(trade.get("price", 0))
                ...
                complement = 1.0 - raw_price
                if abs(complement - reference_price) < abs(raw_price - reference_price):
                    price = complement
                else:
                    price = raw_price

                # Determine our direction from the trade's ``side`` field
                # (which is the *taker's* side).  When queried with
                # maker_address we are the maker — opposite to the taker.
            ...
            return buy_shares, buy_cost, sell_shares, sell_proceeds

        except Exception as e:
            logger.warning(
                f"Failed to query trade costs for {market_id}/{side}: {e}"
            )
            return None
